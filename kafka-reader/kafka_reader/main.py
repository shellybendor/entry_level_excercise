...
async def get_blob_client() -> BlobClient:
    ...
    async with ContainerClient.from_connection_string(conn_str=CONN_STRING,
                                                      container_name=CONTAINER_NAME) as container:
        await container.create_container()
    print("got container, getting blob client")
    blob = BlobClient.from_connection_string(conn_str=CONN_STRING, container_name=CONTAINER_NAME,
                                             blob_name="my_blob")
    return blob
            ...
            async for msg in consumer:
                ...
                if len(message_log) == 5:
                    for m in message_log:
                        await blob_client.upload_blob(m, blob_type="AppendBlob")
